...
<template>
    <div class="container mx-auto px-4 py-8">
        <h1 class="mb-8 text-3xl font-bold">二维码生成</h1>
                ...
                <div class="card-body">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="form-control">
                            ...
                            <input v-model="form.foreground" type="color" class="h-10 w-full" />
                        </div>
                        <div class="form-control">
                            <label class="label">
                                <span class="label-text">背景色</span>
                            </label>
                            <input v-model="form.background" type="color" class="h-10 w-full" />
                        </div>
                    </div>
            ...
            <div class="card bg-base-100 shadow-xl">
                <div class="card-body">
                    ...
                    <div v-if="qrcodeUrl" class="flex flex-col items-center gap-4">
                        <img :src="qrcodeUrl" :alt="form.content" class="max-w-full" />

                        <!-- 下载按钮 -->
                        <div class="flex gap-2">
                            <button class="btn btn-outline" @click="handleDownload('png')">
                                下载 PNG
                            </button>
                            <button class="btn btn-outline" @click="handleDownload('svg')">
                                下载 SVG
                            </button>
                        </div>
                    </div>

                    <div v-else class="flex h-[300px] items-center justify-center text-gray-400">
                        二维码预览区域
                    </div>
                </div>
            </div>
...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        if (logo) {
            ...
            const logoBuffer = await sharp(logo).resize(logoSize, logoSize).toBuffer();
