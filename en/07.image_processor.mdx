...
<template>
    <div class="container mx-auto px-4 py-8">
        <h1 class="mb-8 text-3xl font-bold">图片处理工具</h1>
        ...
        <!-- 图片上传区域 -->
        <div
            class="border-base-300 cursor-pointer rounded-lg border-2 border-dashed p-8 text-center"
            @click="open()"
            @drop.prevent="selectedImages = Array.from($event.dataTransfer?.files || [])"
            @dragover.prevent
        >
            <div class="mb-4 text-lg">点击或拖拽图片到此处</div>
            <div class="text-base-content/60 text-sm">支持 JPG、PNG、WebP 等格式</div>
        </div>

        <!-- 图片预览列表 -->
        <div v-if="selectedImages.length" class="mt-8 grid grid-cols-2 gap-4 md:grid-cols-4">
            <div
        ...
        try {
            ...
            switch (metadata.format) {
                ...
                case "jpg":
                    ...
                    break;
                case "png":
                    processed = await image.png({ quality: quality / 100 }).toBuffer();
                    break;
                case "webp":
            ...
            // 创建压缩后的文件
            const compressedFile = new File([processed], `compressed_${file.name}`, {
                type: file.type,
            });
...
export function useImageConverter() {
    ...
    const convertImage = async (
        ...
        try {
            ...
            switch (format) {
                case "jpeg":
                    processed = await image.jpeg(options).toBuffer();
                    break;
                case "png":
                    processed = await image.png(options).toBuffer();
                    break;
                case "webp":
                    processed = await image.webp(options).toBuffer();
                    break;
            }
            ...
            // 创建转换后的文件
            const convertedFile = new File([processed], newName, { type: `image/${format}` });
...
export function useImageEditor() {
    ...
    const rotateImage = (degree: number) => {
        ...
        ctx.value.drawImage(canvas.value, -width / 2, -height / 2);
        ctx.value.restore();
    };
...
<template>
    <div class="space-y-8">
        ...
        <div class="form-control">
            ...
            <input v-model="quality" type="range" min="0" max="100" class="range" />
        </div>
        ...
        <!-- 编辑工具栏 -->
        <div class="btn-group">
            <button class="btn" @click="rotateImage(-90)">向左旋转</button>
            <button class="btn" @click="rotateImage(90)">向右旋转</button>
        </div>

        <!-- 画布容器 -->
        <div class="bg-base-200 relative aspect-video overflow-hidden rounded-lg">
